...
int main()
{
	...
	Model ourModel(FileSystem::getPath("resources/objects/fighter/idle.dae"));
	Animation idleAnimation(FileSystem::getPath("resources/objects/fighter/idle.dae"), &ourModel);
	Animation dodgeAnimation(FileSystem::getPath("resources/objects/fighter/dodge.dae"), &ourModel);
	//Animation runAnimation(FileSystem::getPath("resources/objects/fighter/dodge.dae"), &ourModel);
	Animation punchAnimation(FileSystem::getPath("resources/objects/fighter/punch.dae"), &ourModel);
	Animation kickAnimation(FileSystem::getPath("resources/objects/fighter/kick.dae"), &ourModel);
	Animator animator(&idleAnimation);
	enum AnimState charState = IDLE;
	float blendAmount = 0.f;
	float blendRate = 0.02f;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS)
			animator.PlayAnimation(&dodgeAnimation, NULL, 0.0f, 0.0f, 0.0f);
		...
		case IDLE:
			if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) {
				blendAmount = 0.0f;
				animator.PlayAnimation(&idleAnimation, &dodgeAnimation, animator.m_CurrentTime, 0.0f, blendAmount);
				charState = IDLE_DODGE;
			}
		...
		case IDLE_DODGE:
			...
			animator.PlayAnimation(&idleAnimation, &dodgeAnimation, animator.m_CurrentTime, animator.m_CurrentTime2, blendAmount);
			if (blendAmount > 0.9f) {
				blendAmount = 0.0f;
				float startTime = animator.m_CurrentTime2;
				animator.PlayAnimation(&dodgeAnimation, NULL, startTime, 0.0f, blendAmount);
				charState = DODGE_IDLE;
			}
			printf("idle_punch\n");
			break;
		case DODGE_IDLE:
			if (animator.m_CurrentTime > 1.0f) {
				blendAmount += blendRate;
				blendAmount = fmod(blendAmount, 1.0f);
				animator.PlayAnimation(&dodgeAnimation, &idleAnimation, animator.m_CurrentTime, animator.m_CurrentTime2, blendAmount);
				if (blendAmount > 0.9f) {
					blendAmount = 0.0f;
					float startTime = animator.m_CurrentTime2;
